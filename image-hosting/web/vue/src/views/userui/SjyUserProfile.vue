      ...
      <div v-if="activeTab === 'explore'">
        ...
        <div v-else class="image-grid masonry-container">
          <div class="image-card" v-for="image in userImages" :key="image.imageId" @click="openImageDetail(image, index)">
            <img :src="MINIO_SERVER_PORT + image.minioUrl" :alt="image.fileName" class="image-thumbnail">
...
import ImageDetailComponent from './components/SjyImageDetailComponent.vue';
import FooterComponent from './components/SjyFooterComponent.vue';
import EchartDataComponent from './components/SjyEchartDataComponent.vue';
...
const fetchUserUploadedImages = async (userId: string) => {
  ...
  error.value = null;
  userImages.value = [];
  profileHeaderBackgroundImage.value = null; 
  ...
  try {
    ...
    if (response.data && response.data.code === 200 && Array.isArray(response.data.data)) {
      ...
      if (userImages.value.length > 0) {
        ...
        profileHeaderBackgroundImage.value = MINIO_SERVER_PORT + userImages.value[randomIndex].minioUrl;
      }
